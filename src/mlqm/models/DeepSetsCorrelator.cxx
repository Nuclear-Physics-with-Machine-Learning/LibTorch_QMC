...
torch::Tensor DeepSetsCorrelatorImpl::forward(torch::Tensor x){
    ...
    auto summed_output = individual_net(x).sum(1);
    ...
    Does summed output need grad?
    PLOG_INFO << summed_output.requires_grad();



    // #pragma omp parallel for reduction(+summed_output)
    at::parallel_for(0, n_particles, 0, [&](int64_t start, int64_t end){
